...
cfgfile = path.join(
    path.expanduser('~'), '.config', 'gmail-download', 'gmail_query.conf')
ext_dict = {
    'eml': '.eml',
    'docx': '.docx',
    'html': '.html',
...
class args_config():
    ...
    def __init__(self, cfgfile, fallback):
        ...
        try:
            self.att_get = cfgparser.getboolean('Setup', 'download_attachments')
        except:
        ...
        try:
            self.sort_case = cfgparser.getboolean(
                'Setup', 'sorting_case_sensitive')
        except:
...
class gmail_query():
    ...
    def __init__(self, outdir, flags=None, cfgfile=cfgfile):
        ...
        credentials = get_credentials(
            app_name, client_secret_file, scopes, flags)
    ...
    def query_todays(self, todays, bdays, first, otype, msize):
        ...
        cols = [
            'threadId', 'body', 'ft_header', 'header', 'date', 'subject', 'fn',
            'att']
        ...
        df.sort_values(
            by=['threadId', 'dtf'], inplace=True, ascending=[True, not first])
    ...
    def sort_query(self, sort_rules, case):
        ...
        for root, dirs, files in outwalk_static:
            if len(files) > 0:
                for fname in files:
                    if apply_rules(srules, outdir, root, fname,
                                   case) in srules.keys():
                        break
    ...
    def parse_msg(self, msg, otype, prefer='text/html', depth=10):
        ...
        head = [
            '**From:** ' + fr, '**To:** ' + to, '**Cc:** ' + cc
            if cc else cc, '**Subject:** ' + sub, '**Date:** ' + dates,
            '**Id:** ' + msg['id'], '**Content-type:** ' + pmime]
        ...
        ft_body = pandoc.convert_text(
            plain, ctype, format='html', extra_args=['--smart'])
    ...
    def get_att(self, msg_id, att_id):
        return self.messages.attachments().get(
